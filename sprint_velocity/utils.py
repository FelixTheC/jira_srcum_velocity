...
def create_initial_json():
    ...
    if not config_file.exists():
        data = {"info": "Config is autogenerated and should not be changed by hand."}
        with config_file.open("w") as file:
            json.dump(data, file)


def get_json_data() -> dict:
    with Path(settings.CONFIG_FILE).open("r") as file:
        try:
...
def save_json_data(json_data: dict):
    with Path(settings.CONFIG_FILE).open("w") as file:
        json.dump(json_data, file)


@match_typing
def get_header(token: str) -> CaseInsensitiveDict[Any]:
...
@match_typing
def get_json_response(base_url: str, *, headers: CaseInsensitiveDict):
    result = request("GET", url=base_url, headers=headers).json()
    return result
