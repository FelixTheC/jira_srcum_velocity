...
def test_settings_project_fails_on_missing_project(tmp_config):
    result = runner.invoke(app, ["settings-add-sprint-to-project", "exampleCorps", "foobar", "7893", "2022-10-10"])
    assert result.exit_code == 1


def test_settings_project_success(tmp_config):
    result = runner.invoke(app, ["settings-project", "exampleCorps", "foobar", "12456abcd", "https://jira.foobar.de"])
